...
## ★ SLAT ENCODER + ENSEMBLE + full-FT — 07-03 (all vs human GT; baseline = head-only FT = 0.650 CV)
Learned EdgeConv encoder over SAM3D's structured latent (models/slat_encoder.py + mlp_slat.py; ~8.8k voxels -> gather to vertices) replacing the flat 8-d SLAT channel. Feature-channel ablation (inference-zeroing on the 5 CV fold models) first showed: DINO is the engine (-0.26), CLIP-VISION is ~dead (-0.008), SLAT props up display (-0.24) + grasp (-0.10), normals help contain/sit. => 3D structure is the lever for the weak verbs; hence the SLAT encoder.

CONFIRMED (5-fold CV, FAIR per-fold baseline + bootstrap; scratchpad/slat_cv_evaluate.py, /tmp/slat_anchor.json):
- SLAT-encoder HEAD-ONLY: trained-mean 0.678 vs 0.650 = +0.029, 95% CI[+0.002,+0.056], P=0.99 (marginal). Big wins contain +0.175, display +0.186; but REGRESSES pour -0.130, move -0.103; grasp ~flat. Strong specialist (geometric localization), weak generalist.
- ★ ENSEMBLE avg(baseline+SLAT logits) = 0.724 vs 0.650 = +0.075, 95% CI[+0.058,+0.091], P=1.00. ROBUST FREE WIN (two existing models). Keeps SLAT's contain/display, RECOVERS pour/move, and LIFTS grasp to 0.415 (+0.093 — above BOTH models, variance reduction). Render: outputs/renders/slat_gains.png (SLAT = structured interior/opening heatmaps vs baseline's diffuse fields).
VARIANT SCREENS (fold-0 vs SAME-fold baseline 0.666): drop_clip 0.658 NOT a win (CLIP-vision not dead here; pour tanks); bigger-encoder(h128 l4) 0.681 NOT a win.

★★ full-FT (unfreeze the whole trunk, --full_ft --lr 3e-4): 3-fold CV trained-mean = 0.823 (+0.17 vs baseline-5fold-mean 0.650), holds from fold-0 0.832 => NOT overfitting. EVERY verb up, no regressions: contain .954 display .867 sit .880 move .832 pour .812 GRASP .594 (+0.273 — the verb NOTHING else cracked: base .32, ensemble .42). Biggest result of the session IF it survives the confound.
CONFOUND (UNRESOLVED): the baseline (0.650) was HEAD-ONLY FT; full_ft unfreezes the trunk, so +0.17 may be FULL-FINE-TUNING itself, not the SLAT encoder — we never full-FT'd the plain baseline. Isolation check (baseline full-FT, NO SLAT, outputs/basefull_fold0) queued, gated on a free GPU (both GPUs taken by other users ~00:50). Either way, full-FT >> head-only-FT is a MAJOR finding (esp grasp), and reframes the whole session's head-only baseline.
METHOD (banked): screen fold-0 vs SAME-fold baseline (never the k-fold mean), confirm winners multi-fold + bootstrap. train_slat_cv.py flags: --full_ft --drop_clip --enc_hidden --enc_layers.
NEXT: (1) resolve full-FT-vs-SLAT confound (baseline full-FT). (2) if SLAT beats baseline-full-FT -> 5-fold full-FT + fair CI = new headline. (3) reinit-fix variant + part-aware DINO keys = deferred. Nothing committed.

### full-FT characterization (CPU stats, no GPU) — 07-03 07:1x
Ran on the existing cv_ft3_fold{0,1,2}.pt while the box stayed GPU-saturated (both A40s taken by other users all night). Establishes that 0.823 is real and generalizes, independent of the still-pending confound:
- BOOTSTRAP CI on the full-FT trained-mean: 0.823, 95% CI [0.800, 0.843] (n_obj=135, 4000 resamples). Tight — not noise.
- GAIN over head-only baseline (same fold-0/1/2 objects, paired bootstrap): +0.160, 95% CI [+0.134, +0.185], P(gain>0)=1.00. (Still CONFOUNDED = full-FT+SLAT vs head-only; not yet split into trunk-unfreezing vs SLAT-encoder.)
- OVERFITTING (train-vs-val gap, each fold model scored on its own train objs vs held-out val, identical settings): trained-mean train 0.866 / val 0.823 / GAP +0.043. Per-verb: contain +0.013, move -0.006, pour +0.035, sit +0.036, display +0.058, GRASP +0.122 (biggest — the sparse/appearance-hard verb fits train objs more, but held-out grasp 0.596 still > GEAL 0.472). => generalizes to unseen INSTANCES OF SEEN CATEGORIES; not memorizing.
- LEAKAGE audit of the CV folds: 0 exact-object overlap and 0 same-source-different-version overlap across all 3 folds (106 sources have multiple reconstruction versions; none straddle train/val). Every val category also in train (val-only-cats=0) -> instance-level generalization, NOT unseen-category/unseen-verb.
- PER-FOLD means (independent splits): 0.834 / 0.843 / 0.790 -> not fold-lucky. grasp low every fold (0.62/0.67/0.49) -> learning real structure, not memorizing labels.
Renders: outputs/renders/fullft_best.png (best object/verb) + fullft_median.png (median/typical object/verb), both scored out-of-fold.
STILL PENDING (the only GPU-gated piece): baseline full-FT (no SLAT) on fold-0 -> decompose the +0.16 into trunk-unfreezing vs SLAT-encoder. Then 5-fold full-FT + bootstrap if SLAT wins.
