"""PRELIMINARY full 5-fold (rich from ep13/14 snapshots): held-out synonyms.
RICH loaded from /tmp/rich_snap_fold{k}.pt ; baseline from outputs/gnn_synaug8. CPU."""
...
TPL=["{w}","you {w} this object","the part where you {w}","an action to {w}"]
SUB=20000
...
def load_set(fmt):
    d={}
    for k in range(5):
        cg=torch.load(fmt.format(k=k),map_location="cpu",weights_only=False)
        gc=AffordanceGNNConfig(**cg["cfg"]);m=AffordanceGNN(gc);m.load_state_dict(cg["model"]);m.eval();d[k]=(m,gc)
    return d
BASE=load_set("outputs/gnn_synaug8_fold{k}.pt"); RICH=load_set("/tmp/rich_snap_fold{k}.pt")
...
emb_exact={v:enc(v) for v in HELD}; emb_hbare={v:enc(HELD[v]) for v in HELD}
_c={}
def pred(mset,o,e):
    if o not in _c:
        ...
        k=obj2fold[o];knn=AffordanceGNN._build_knn(it["vertex_positions"].float()[idx],mset[k][1].knn_k);_c[o]=(it,sel,idx,knn,k)
    it,sel,idx,knn,k=_c[o];m,gc=mset[k];sl=lambda kk:(it.get(kk).float()[idx] if it.get(kk) is not None else None);z=lambda d:torch.zeros(len(sel),d)
    with torch.no_grad():
...
print("=== PRELIMINARY full 5-fold (rich ep13/14 snapshots): held-out synonyms ===")
print(f"{'verb':9s} {'held-out':>11s} {'exact':>6s} {'base-bare':>10s} {'RICH-bare':>10s}")
EX,BB,RB=[],[],[]
for v in HELD:
    ex=[];bb=[];rb=[]
    for o in sorted(obj2fold):
        ...
        p0=pred(RICH,o,emb_exact[v]);_,sel,_,_,_=_c[o];y=(h[sel]>=0.5).astype(int)
        if not(0<y.sum()<len(y)):continue
        ex.append(average_precision_score(y,p0))
        bb.append(average_precision_score(y,pred(BASE,o,emb_hbare[v])))
        rb.append(average_precision_score(y,pred(RICH,o,emb_hbare[v])))
    if ex:
        EX.append(np.mean(ex));BB.append(np.mean(bb));RB.append(np.mean(rb))
        print(f"{v:9s} {HELD[v]:>11s} {np.mean(ex):6.3f} {np.mean(bb):10.3f} {np.mean(rb):10.3f}")
if EX:
    print(f"\n{'MACRO':9s} {'':>11s} {np.mean(EX):6.3f} {np.mean(BB):10.3f} {np.mean(RB):10.3f}")
    print(f"held-out drop vs exact:  base-bare {np.mean(BB)-np.mean(EX):+.3f}   RICH-bare {np.mean(RB)-np.mean(EX):+.3f}")
