...
run() {  # $1=tag $2=extra
  for attempt in $(seq 6); do
    : > /tmp/gnn_${1}.log
    CUDA_VISIBLE_DEVICES=0 nice -n 10 timeout 3600 .venv-affordance/bin/python "$SCR/train_gnn_cv.py" \
...
run bigedge "--gnn_layers 4 --gnn_hidden 192 --knn_k 16 --edge_geom"
run k24     "--knn_k 24"
run bigk24  "--gnn_layers 4 --gnn_hidden 192 --knn_k 24"
echo "GNN5 DONE" >> /tmp/gnn_launch.log
